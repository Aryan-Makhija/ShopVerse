...
//         const keywords = query.split(/\s+/); // Split query into words

//         // Create regex for each keyword (case-insensitive)
//         const regexKeywords = keywords.map((word) => new RegExp(word, 'i'));

//         // Step 1: Search across all related models
//         const [
//             matchedProducts,
//             matchedBrands,
//             matchedCategories,
...
//             ProductVariant.find({ price: { $in: regexKeywords } }),
//             Attribute.find({
//                 $or: [{ size: { $in: regexKeywords } }, { color: { $in: regexKeywords } }]
//             })
//         ]);

//         // Step 2: Collect all matching productIds from related models
...
//         matchedVariants.forEach((v) => matchedIds.add(v.productId.toString()));

//         // Step 3: Find unique matching products
//         const products = await ProductInfo.find({ _id: { $in: Array.from(matchedIds) } });
